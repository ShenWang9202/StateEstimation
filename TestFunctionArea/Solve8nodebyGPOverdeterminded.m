...
for i = 1:1
    ...
    while (IterateError > 0.1)
        ...
        cvx_begin
        variables W(NumberofX) x
        % objective function is the box volume
        minimize( 0.1*x^2) % suppose delta_h2  is much more accurate.
        subject to
        ...
        %% Tank and Reservoir
        [CvxExpFixed,FixedValueVector] = FixedConstLinear(W,X0,SettingsNStatus,IndexInVar);
        FixedValueVector
        ...
        MeasuredHeadValue = [876.89 863.49];
        [~,n] = size(MeasuredHeadValue);
        varIndex =[];
        for  s_ind = 1:n
            var_index = find(strcmp(Variable_Symbol_Table,MeasuredHeadLable{s_ind}))
            varIndex = [varIndex;var_index];
        end
        W(varIndex(1)) - W(varIndex(2)) == MeasuredHeadValue(1) - MeasuredHeadValue(2) + x
%         
